    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                @if(Auth::user()->role == 0 || user_has_permission(Auth::user(), "create-category"))
                ...
                @if(count($categories) > 0)
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            @if(Auth::user()->role == 0 || user_has_permission(Auth::user(), "show-category"))<button title="Lihat Produk Untuk Kategori Ini" type="button" class="btn btn-primary btn-xs" onclick="window.location.href='/products?category={{ $d->category_id }}'"><i class="fas fa-external-link-alt"></i></button>@endif
                            @if(Auth::user()->role == 0 || user_has_permission(Auth::user(), "edit-category"))<button title="Edit Shelf" type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#add-category" onclick="editCategory({{ json_encode($data) }})"><i class="fas fa-edit"></i></button>@endif
                            @if(Auth::user()->role == 0 || user_has_permission(Auth::user(), "delete-category"))<button title="Hapus Produk" type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#delete-category" onclick="deleteCategory({{ json_encode($data) }})"><i class="fas fa-trash"></i></button>@endif
                        </td>
                    </tr>
                    @endforeach
                @else
