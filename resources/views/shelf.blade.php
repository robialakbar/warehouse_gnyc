...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="card-header">
            @if(Auth::user()->role == 0 || user_has_permission(Auth::user(), "create-shelf"))<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add-shelf" onclick="addShelf()"><i class="fas fa-plus"></i> Add New Shelf</button>@endif
        </div>
        <div class="card-body">
            ...
            @if(count($shelf) > 0)
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @if(Auth::user()->role == 0 || user_has_permission(Auth::user(), "edit-shelf"))<button title="Edit Shelf" type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#add-shelf" onclick="editShelf({{ json_encode($data) }})"><i class="fas fa-edit"></i></button>@endif
                        @if(Auth::user()->role == 0 || user_has_permission(Auth::user(), "delete-shelf"))<button title="Hapus Shelf" type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#delete-shelf" onclick="deleteShelf({{ json_encode($data) }})"><i class="fas fa-trash"></i></button>@endif
                    </td>
                </tr>
                @endforeach
            @else
